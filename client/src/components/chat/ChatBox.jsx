...
const ChatBox = () => {
    ...
    const { currentChat, isMessagesLoading, sendTextMessage, messages } = useContext(ChatContext);
    ...
    return <Stack gap={4} className="chat-box">
        ...
        <Stack gap={3} className="messages">
            {messages && messages.map((m, index) => {
                return (
                    <Stack key={index} className={m.senderId === user?.id ? "message self align-self-end flex-grow-0" :
